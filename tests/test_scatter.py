...
def test_scatter(test_df):
    ...
    assert plot_dict['vconcat'][1]['hconcat'][0]['encoding']['x']['type'] == \
        "quantitative"
    assert plot_dict['vconcat'][1]['hconcat'][0]['encoding']['y']['type'] == \
        "quantitative"
    assert plot_dict['vconcat'][1]['hconcat'][1]['encoding']['x']['type'] == \
        "quantitative"
    assert plot_dict['vconcat'][1]['hconcat'][1]['encoding']['y']['type'] == \
        "quantitative"

    # example 2
    plot_two = sc.make_scatter(test_df, xval="Displacement",
                               yval="Weight_in_lbs",
                               x_transform=True,
                               y_transform=True)
    ...
    assert np.log(test_df['Displacement']).min(
    ) == plot_dict['vconcat'][1][
        'hconcat'][0]['encoding']['x']['scale']['domain'][0]

    assert np.log(test_df['Displacement']).max(
    ) == plot_dict['vconcat'][1][
        'hconcat'][0]['encoding']['x']['scale']['domain'][1]

    assert np.log(test_df['Weight_in_lbs']).min(
    ) == plot_dict['vconcat'][1][
        'hconcat'][0]['encoding']['y']['scale']['domain'][0]

    assert np.log(test_df['Weight_in_lbs']).max(
    ) == plot_dict['vconcat'][1][
        'hconcat'][0]['encoding']['y']['scale']['domain'][1]
