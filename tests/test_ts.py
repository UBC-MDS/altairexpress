...
def test_ts():
    ...
    assert isinstance(ts.ts_alt(test_df1, "earnings", 4),
                      alt.vegalite.v3.api.Chart), \
        "Plot type is not an Altair object."
    assert isinstance(ts.ts_alt(test_df2, "temp", 1),
                      alt.vegalite.v3.api.Chart), \
        "Plot type is not an Altair object."
    assert ts.ts_alt(test_df1, "earnings", 4).to_dict()[
                     'mark'] == "line", "The plot should be line charts."
    assert ts.ts_alt(test_df2, "temp", 1).to_dict()[
                     'mark'] == "line", "The plot should be line charts."
    ...
    with pytest.raises(Exception) as e:
        assert ts.ts_alt(test_df1, 2, 4)
    assert str(
        e.value) == "TypeError: The column name must be entered as a string."

    with pytest.raises(Exception) as e:
        assert ts.ts_alt(test_df1, "earnings", "4")
    assert str(
        e.value) == "ValueError: Frequency must be an integer from {1, 4, 12, \
            52}."

    with pytest.raises(Exception) as e:
        assert ts.ts_alt(test_df1, "earnings", 6)
    assert str(
        e.value) == "ValueError: Frequency must be an integer from {1, 4, 12, \
            52}."

    with pytest.raises(Exception) as e:
        assert ts.ts_alt(test_df1, "earning", 4)
    assert str(
