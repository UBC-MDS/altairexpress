...
def hist(data, variable):
    ...
    annotation2 = (alt.Chart(data)
                   .mark_text(color='red', text="Median is {0:.2f}"
                              .format(variable_median))
                   .encode(alt.X("x_val:Q"), alt.Y("y_val:Q"))
                   .transform_calculate(x_val=str(annotation_x),
                                        y_val=str(np.median(np.arange(0,
                                                                      500)))))

    annotation3 = (alt.Chart(data)
                   .mark_text(color='blue', text="Standard Dev is {0:.2f}"
                              .format(variable_sd))
                   .encode(alt.X("x_val:Q"), alt.Y("y_val:Q"))
                   .transform_calculate(x_val=str(annotation_x),
                                        y_val=str(np.std(np.arange(0,
                                                                   400)))))
